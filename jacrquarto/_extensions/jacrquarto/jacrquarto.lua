-- jacrquarto.lua  (v0.1.0)
-- Journal of the Association for Consumer Research Quarto/Pandoc filter
-- Modes: "manuscript" (default) | "article"

local footnote_count = 0
local jacr_mode = "manuscript"
...
-- =========================================================================
function Meta(meta)
  if meta["jacr-mode"] then
    jacr_mode = pandoc.utils.stringify(meta["jacr-mode"])
  end
  ...
  saved_meta.editor = meta["jacr-editor"] and safe_str(meta["jacr-editor"]) or ""
  saved_meta.pub_info = meta["jacr-publication-info"] and meta_to_latex(meta["jacr-publication-info"]) or ""
  saved_meta.author_note = meta["jacr-author-note"] and meta_to_latex(meta["jacr-author-note"]) or ""

  -- Article mode: override metadata for publication-style single-column layout
  if is_latex and jacr_mode == "article" then
    meta["jacr-article-mode"] = pandoc.MetaBool(true)
  ...
  -- Validation against JACR guidelines
  -- (JACR does not specify a keyword count, so keywords are optional and
  -- rendered only when supplied.)
  if saved_meta.abstract_plain ~= "" then
    local wc = count_words(saved_meta.abstract_plain)
    if wc >= 150 then
      log_warning("JACR: Abstract should have fewer than 150 words. Current: ~" .. wc)
    end
  end
...
local function build_article_header()
  ...
  table.insert(lines, "\\jacrsetuparticle")
  table.insert(lines, "")

  -- Issue volume and number (optional; JACR heading-page item 1)
  if saved_meta.pub_info ~= "" then
    table.insert(lines, "{\\noindent\\small " .. saved_meta.pub_info .. "\\par}%")
...
function Note(el)
  ...
  if footnote_count == 5 then
    log_warning("JACR: Footnotes are discouraged; a maximum of 4 is allowed.")
  end
  return el
end
...
function Div(el)
  ...
  if el.classes:includes("jacr-label") then
    if is_latex then
      local blocks = pandoc.Blocks({})
  ...
  -- Table/figure notes: small indented text below table
  if el.classes:includes("table-notes") then
    if is_latex then
      local blocks = pandoc.Blocks({})
      blocks:insert(pandoc.RawBlock("latex", "\\begin{jacrnotes}"))
      for _, b in ipairs(el.content) do blocks:insert(b) end
      blocks:insert(pandoc.RawBlock("latex", "\\end{jacrnotes}"))
      return blocks
    elseif is_docx then
      local blocks = pandoc.Blocks({})
      for _, b in ipairs(el.content) do
...
function Para(el)
  ...
  if text:sub(1, 5) == "NOTE." then
    if is_latex then
      return pandoc.Blocks({
        pandoc.RawBlock("latex", "\\begin{jacrfigurenote}"),
        el,
        pandoc.RawBlock("latex", "\\end{jacrfigurenote}"),
      })
    end
  end
  return el
...
function Pandoc(doc)
  ...
  if is_latex then
    if jacr_mode == "article" then
      ...
      local ms = {}
      table.insert(ms, "\\jacrsetupmanuscript")
      if saved_meta.title ~= "" then
        table.insert(ms, "\\begin{center}")
        table.insert(ms, "{\\normalsize\\bfseries " .. saved_meta.title .. "}")
